  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        <StatCard label="Total sales" value={formatINR(ledger.totalSales)} />
        <StatCard label="Received" value={formatINR(ledger.totalReceived)} tone="success" />
        <StatCard label="Outstanding" value={formatINR(ledger.outstanding)} tone="danger" />
      </div>
          ...
          <tbody>
            {ledger.entries.map((e: any) => (
              <tr key={e.id} className="border-b border-[var(--border)] last:border-0">
                ...
                <td className={e.entryType === "SALE" ? "text-[var(--danger)]" : "text-[var(--primary)]"}>
                  {e.entryType === "SALE" ? "Invoice" : "Payment received"}
                </td>
